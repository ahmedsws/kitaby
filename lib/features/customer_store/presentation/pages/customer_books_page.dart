...
class CustomerBooksPage extends StatefulWidget {
  const CustomerBooksPage({super.key});

  @override
  State<CustomerBooksPage> createState() => _CustomerBooksPageState();
}

class _CustomerBooksPageState extends State<CustomerBooksPage> {
  String searchText = '';
  Future<List<BookModel>> getCustomersBooks() async {
    final currentUser = await Constants.getUser();
  ...
  Widget build(BuildContext context) {
    ...
    return Directionality(
      ...
      child: Scaffold(
        appBar: const BaseAppBar(title: 'كتبي'),
        body: Padding(
                              ...
                              ? GridView.count(
                                  ...
                                  children: [
                                    ...snapshot.data!.map(
                                      ...
                                      (doc) =>
                                          doc.book.title.contains(searchText),
                                    ),
                                  ],
                                )
